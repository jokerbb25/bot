...
def execute_trade(
    ...
    if result is None:
        logger.error("🔴 ORDER FAILED [%s] → retcode=NO_RESULT", symbol)
        logger.error("MT5 returned NULL result (order not sent).")
        sys.stdout.flush()
        gui.push_status("ORDER_REJECTED")
        return None, None
    logger.info("MT5 RESPONSE: %s", result)
    retcode = getattr(result, "retcode", None)
    if retcode != mt5.TRADE_RETCODE_DONE:
        comment = getattr(result, "comment", "")
        logger.error("🔴 ORDER FAILED [%s] → retcode=%s", symbol, retcode)
        if comment:
            logger.error("ORDER_REJECTED (%s) %s", retcode, comment)
        sys.stdout.flush()
        gui.push_status("ORDER_REJECTED")
        return result, None
    ticket = getattr(result, "order", 0) or getattr(result, "deal", 0) or getattr(result, "position", 0)
    logger.info("✅ ORDER SENT [%s] ticket=%s", symbol, ticket)
    sys.stdout.flush()
    with position_lock:
        trade_state.update(
            {
