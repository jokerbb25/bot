...
class BotEngine:
    ...
    def _handle_closed_trade(self, ticket: int, pos_data: Dict[str, Any]) -> None:
        history = mt5.history_deals_get(time.time() - 86400, time.time(), ticket=ticket)

        pnl_value: Optional[float] = None
        if history:
            pnl_value = sum([deal.profit for deal in history])

        if pnl_value is None:
            pnl_value = 0.0

        result = "WIN" if pnl_value > 0 else "LOSS"
        msg = f"✅ Trade closed [{pos_data['symbol']}] → Result: {result} | PnL: {pnl_value:.2f}"
        self.logger.log(msg)
        self.send_telegram(msg)

        if self.gui and hasattr(self.gui, "update_trade_result"):
            try:
    ...
    def scan_symbol(self, symbol: str) -> None:
        ...
        try:
            ...
            if (
                ...
                self.logger.log(f"[{symbol}] ⚠️ Trade skipped → RSI and EMA in conflict")
                return
