...
def _patch_iqoption_underlying() -> None:
    ...
    safe_get_digital_underlying_list_data.__name__ = (
        "safe_get_digital_underlying_list_data"
    )
...
class SafeIQOption(IQ_Option):
    ...
    def sync_active_catalog(self, open_time_payload: Optional[Dict] = None) -> None:
        ...
        def register(
            symbol: Optional[str], active_id: Optional[Union[int, str]]
        ) -> None:
            if not symbol:
                return
            try:
    ...
    def resolve_active_symbol(
        self, active_id: Optional[Union[int, str]]
    ) -> Optional[str]:
...
class BotWorker(QObject):
    ...
    def _filtrar_activos_operables(self, iq, pares: List[TradePair]) -> List[TradePair]:
        pares_operables = []
        for par in pares:
            if self._stop_event.is_set():
                break
        ...
        for alias in aliases:
            ...
            if not df.empty:
                if objetivo != par.api_symbol:
                    logging.debug(
                        "Velas para %s obtenidas usando alias %s.",
                        par.display,
                        objetivo,
                    )
                return df
        ...
        try:
            if par.instrument_type == "digital":
                ok, op_id = iq.buy_digital_spot(
                    par.api_symbol, MONTO, senal, EXPIRACION
                )
                tipo = "digital"
            else:
...
class BotGUI(QWidget):
    ...
    def on_trade_completed(
        self, par: str, resultado: str, pnl: float, pnl_total: float
    ) -> None:
        r = self.ensure_row(par)
